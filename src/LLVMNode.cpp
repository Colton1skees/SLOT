...
namespace SLOT
{
    ...
    expr LLVMArgument::ToSMT(LLVM_CACHE& cache)
    {
        if (cache.count(contents)) return expr(scx, cache.at(contents));
        return expr(scx, cache[contents] = function.variables.at(contents->getName().str()));
    }
    ...
    expr LLVMConstant::ToSMT(LLVM_CACHE& cache)
    {
        if (cache.count(contents)) return expr(scx, cache.at(contents));
        expr res(scx);
        if (SMTSort().is_bool())
        {
            res = scx.bool_val(((Constant*)contents)->isOneValue());
        }
        else if (SMTSort().is_bv())
        {
            SmallString<CONSTANT_STRING_MAX_WIDTH> str;
            ((Constant*)contents)->getUniqueInteger().toString(str,10,false);
            res = scx.bv_val(str.c_str(), Width());
        }
        else //FPA case
        {
            //bitcastToAPInt
            ...
            res = scx.bv_val(str.c_str(), Width()).mk_from_ieee_bv(SMTSort());
        }
        return expr(scx, cache[contents] = res);
    }
    ...
    expr LLVMIntrinsicCall::ToSMT(LLVM_CACHE& cache)
    {
        if (cache.count(contents)) return expr(scx, cache.at(contents));
        auto logic = [this, &cache]() 
        {
            ...
            switch (id)
            {
                case Intrinsic::is_fpclass:
                    ...
                    return LLVMIntrinsicCall::FPClassCheck(scx, Child(0)->ToSMT(cache), bits);
                /*case Intrinsic::round:
                    return z3::round_fpa_to_closest_integer(Child(0)->ToSMT(cache));*/
                case Intrinsic::abs:
                    arg = Child(0)->ToSMT(cache);
                    return ite(arg < Zero(), -arg, arg);
                case Intrinsic::smin:
                    left = Child(0)->ToSMT(cache);
                    right = Child(1)->ToSMT(cache);
                ...
                case Intrinsic::fabs:
                    return z3::abs(Child(0)->ToSMT(cache));
                case Intrinsic::fma:
                    return z3::fma(Child(0)->ToSMT(cache),Child(1)->ToSMT(cache),Child(2)->ToSMT(cache), scx.fpa_rounding_mode());
                case Intrinsic::sqrt:
                    return z3::sqrt(Child(0)->ToSMT(cache), scx.fpa_rounding_mode());
                case Intrinsic::minnum:
                    return min(Child(0)->ToSMT(cache),Child(1)->ToSMT(cache));
                case Intrinsic::maxnum:
                    return max(Child(0)->ToSMT(cache),Child(1)->ToSMT(cache));
                case Intrinsic::bswap:
                    //Reverse the order of bytes
                    arg = Child(0)->ToSMT(cache);
                    {
                        expr_vector v_inner(scx);
                        for (int i = (Width()/8)-1; i>=0; i--)
                        {
                            v_inner.push_back(arg.extract(Width()-(8*i)-1, Width()-(8*(i+1))));
                        }
                        return concat(v_inner);
                    }
                ...
                case Intrinsic::bitreverse:
                    ...
                    arg = Child(0)->ToSMT(cache);
                    {
                        ...
                        {
                            v_inner.push_back(arg.extract(i,i));
                        }
                        return concat(v_inner);
                    }
                case Intrinsic::fshl:
                    //Funnel shift
                    return shl(concat(Child(0)->ToSMT(cache), Child(1)->ToSMT(cache)), zext(Child(2)->ToSMT(cache), Width())).extract((Width() * 2) - 1, Width());
                case Intrinsic::fshr:
                    //Funnel shift
                    return lshr(concat(Child(0)->ToSMT(cache),Child(1)->ToSMT(cache)),Child(2)->ToSMT(cache)).extract(Width()-1,0);
                ...
                case Intrinsic::roundeven:
                    return expr(scx, Z3_mk_fpa_round_to_integral(scx, expr(scx, Z3_mk_fpa_rne(scx)), Child(0)->ToSMT(cache)));
                case Intrinsic::lround:
                    return expr(scx, Z3_mk_fpa_round_to_integral(scx, expr(scx, Z3_mk_fpa_rna(scx)), Child(0)->ToSMT(cache)));
                case Intrinsic::ceil:
                    return expr(scx, Z3_mk_fpa_round_to_integral(scx, expr(scx, Z3_mk_fpa_rtp(scx)), Child(0)->ToSMT(cache)));
                case Intrinsic::floor:
                    return expr(scx, Z3_mk_fpa_round_to_integral(scx, expr(scx, Z3_mk_fpa_rtn(scx)), Child(0)->ToSMT(cache)));
                case Intrinsic::trunc:
                    return expr(scx, Z3_mk_fpa_round_to_integral(scx, expr(scx, Z3_mk_fpa_rtz(scx)), Child(0)->ToSMT(cache)));
                case Intrinsic::experimental_constrained_uitofp:
                    return expr(scx, Z3_mk_fpa_to_fp_unsigned(scx, AsRoundingMode(1), Child(0)->ToSMT(cache), SMTSort()));
                case Intrinsic::experimental_constrained_sitofp:
                    return expr(scx, Z3_mk_fpa_to_fp_signed(scx, AsRoundingMode(1), Child(0)->ToSMT(cache), SMTSort()));
                case Intrinsic::experimental_constrained_fpext:
                case Intrinsic::experimental_constrained_fptrunc:
                    return expr(scx, Z3_mk_fpa_to_fp_float(scx, AsRoundingMode(1), Child(0)->ToSMT(cache), SMTSort()));
                case Intrinsic::experimental_constrained_fadd:
                    return expr(scx,Z3_mk_fpa_add(scx, AsRoundingMode(2), Child(0)->ToSMT(cache), Child(1)->ToSMT(cache)));
                case Intrinsic::experimental_constrained_fsub:
                    return expr(scx,Z3_mk_fpa_sub(scx, AsRoundingMode(2), Child(0)->ToSMT(cache), Child(1)->ToSMT(cache)));
                case Intrinsic::experimental_constrained_fmul:
                    return expr(scx,Z3_mk_fpa_mul(scx, AsRoundingMode(2), Child(0)->ToSMT(cache), Child(1)->ToSMT(cache)));
                case Intrinsic::experimental_constrained_fdiv:
                    return expr(scx,Z3_mk_fpa_div(scx, AsRoundingMode(2), Child(0)->ToSMT(cache), Child(1)->ToSMT(cache)));
                case Intrinsic::experimental_constrained_fma:
                    return expr(scx,Z3_mk_fpa_fma(scx, AsRoundingMode(3), Child(0)->ToSMT(cache), Child(1)->ToSMT(cache), Child(2)->ToSMT(cache)));
                case Intrinsic::experimental_constrained_sqrt:
                    return expr(scx, Z3_mk_fpa_sqrt(scx, AsRoundingMode(1), Child(0)->ToSMT(cache)));
                default:
                    throw UnsupportedLLVMOpException("unsupported intrinsic function", contents);
            }
        };
        return expr(scx, cache[contents] = logic());
    }
    ...
    expr LLVMIcmp::ToSMT(LLVM_CACHE& cache)
    {
        if (cache.count(contents)) return expr(scx, cache.at(contents));
        auto logic = [this, &cache]()
        {
            ...
            if (Predicate()==CmpInst::ICMP_EQ)
            {
                ...
                {
                    return std::unique_ptr<LLVMExpression>{static_cast<LLVMExpression*>(Child(0).release())}->Child(0)->ToSMT(cache) == std::unique_ptr<LLVMExpression>{static_cast<LLVMExpression*>(Child(1).release())}->Child(0)->ToSMT(cache);
                }
                else if (isa<BitCastInst>(vi->getOperand(0)))
                {
                    expr left = std::unique_ptr<LLVMExpression>{static_cast<LLVMExpression*>(Child(0).release())}->Child(0)->ToSMT(cache);
                    expr right = Child(1)->ToSMT(cache).mk_from_ieee_bv(left.get_sort());
                    return left == right;
                }
                else if (isa<BitCastInst>(vi->getOperand(1)))
                {
                    //Reverse order so we can do right.get_sort()
                    expr right = std::unique_ptr<LLVMExpression>{static_cast<LLVMExpression*>(Child(1).release())}->Child(0)->ToSMT(cache);
                    expr left = Child(0)->ToSMT(cache).mk_from_ieee_bv(right.get_sort());
                    return left == right;
                }
                else
                { 
                    //Regular case
                    return Child(0)->ToSMT(cache) == Child(1)->ToSMT(cache);
                }
            }
            ...
            else
            {
                ...
                expr left = Child(0)->ToSMT(cache);
                expr right = Child(1)->ToSMT(cache);
    ...
    expr LLVMFcmp::ToSMT(LLVM_CACHE& cache)
    {
        if (cache.count(contents)) return expr(scx, cache.at(contents));
        auto logic = [this, &cache]()
        {
    ...
    expr LLVMExpression::ToSMT(LLVM_CACHE& cache)
    {
        if (cache.count(contents)) return expr(scx, cache.at(contents));
        auto logic = [this, &cache]()
        {
        ...
        switch (Opcode()) // Instructions
        {
            ...
            case Instruction::ExtractValue:
                ...
                {
                    expr left = std::unique_ptr<LLVMExpression>{static_cast<LLVMExpression*>(Child(0).release())}->Child(0)->ToSMT(cache);
                    expr right = std::unique_ptr<LLVMExpression>{static_cast<LLVMExpression*>(Child(0).release())}->Child(1)->ToSMT(cache);
            ...
            case Instruction::FPToUI:
                ...
                if (isa<CallInst>(child))
                {
                    ...
                    {
                        case Intrinsic::roundeven:
                            return expr(scx,Z3_mk_fpa_to_ubv(scx, expr(scx, Z3_mk_fpa_rne(scx)), Child(0)->ToSMT(cache), Width()));
                        case Intrinsic::lround:
                            return expr(scx,Z3_mk_fpa_to_ubv(scx, expr(scx, Z3_mk_fpa_rna(scx)), Child(0)->ToSMT(cache), Width()));
                        case Intrinsic::ceil:
                            return expr(scx,Z3_mk_fpa_to_ubv(scx, expr(scx, Z3_mk_fpa_rtp(scx)), Child(0)->ToSMT(cache), Width()));
                        case Intrinsic::floor:
                            return expr(scx,Z3_mk_fpa_to_ubv(scx, expr(scx, Z3_mk_fpa_rtn(scx)), Child(0)->ToSMT(cache), Width()));
                        default:
                            break;
                        }
                }
                //default behavior of fptoui is round towards zero
                return expr(scx,Z3_mk_fpa_to_ubv(scx, expr(scx, Z3_mk_fpa_rtz(scx)), Child(0)->ToSMT(cache), Width()));
            case Instruction::FPToSI:
                ...
                if (isa<CallInst>(child))
                {
                    ...
                    {
                        case Intrinsic::roundeven:
                            return expr(scx,Z3_mk_fpa_to_sbv(scx, expr(scx, Z3_mk_fpa_rne(scx)), Child(0)->ToSMT(cache), Width()));
                        case Intrinsic::lround:
                            return expr(scx,Z3_mk_fpa_to_sbv(scx, expr(scx, Z3_mk_fpa_rna(scx)), Child(0)->ToSMT(cache), Width()));
                        case Intrinsic::ceil:
                            return expr(scx,Z3_mk_fpa_to_sbv(scx, expr(scx, Z3_mk_fpa_rtp(scx)), Child(0)->ToSMT(cache), Width()));
                        case Intrinsic::floor:
                            return expr(scx,Z3_mk_fpa_to_sbv(scx, expr(scx, Z3_mk_fpa_rtn(scx)), Child(0)->ToSMT(cache), Width()));
                        default:
                            break;
                        }
                }
                //default behavior of fptosi is round towards zero
                return expr(scx,Z3_mk_fpa_to_sbv(scx, expr(scx, Z3_mk_fpa_rtz(scx)), Child(0)->ToSMT(cache), Width()));
            case Instruction::FPExt:
            case Instruction::FPTrunc:
                return fpa_to_fpa(Child(0)->ToSMT(cache), SMTSort());
            case Instruction::SIToFP:
                return sbv_to_fpa(Child(0)->ToSMT(cache), SMTSort());
            case Instruction::UIToFP:
                return ubv_to_fpa(Child(0)->ToSMT(cache), SMTSort());
            //Unary bitvector instructions
            case Instruction::BitCast:
                //Bitcast from bitvector to floating point
                if(SMTSort().is_fpa())
                {
                    return Child(0)->ToSMT(cache).mk_from_ieee_bv(SMTSort());
                }
                else
                {
                    ...
                    return function.AddBCVariable(Child(0), cache);
                }
            case Instruction::Trunc:
                return Child(0)->ToSMT(cache).extract(Width()-1, 0);
            ...
            case Instruction::LShr:
                return lshr(Child(0)->ToSMT(cache),Child(1)->ToSMT(cache));
            case Instruction::AShr:
                return ashr(Child(0)->ToSMT(cache),Child(1)->ToSMT(cache));
            case Instruction::UDiv:
                return udiv(Child(0)->ToSMT(cache),Child(1)->ToSMT(cache));
            case Instruction::URem:
                return urem(Child(0)->ToSMT(cache),Child(1)->ToSMT(cache));
            case Instruction::SRem:
                return srem(Child(0)->ToSMT(cache),Child(1)->ToSMT(cache));
            ...
            case Instruction::FRem:
                return rem(Child(0)->ToSMT(cache), Child(1)->ToSMT(cache));
            //Select instruction
            case Instruction::Select:
                return ite(Child(0)->ToSMT(cache),Child(1)->ToSMT(cache),Child(2)->ToSMT(cache));
            default:
